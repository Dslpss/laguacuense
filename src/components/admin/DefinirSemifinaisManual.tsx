...
export function DefinirSemifinaisManual({
  ...
  const [dataJogo1, setDataJogo1] = useState<string>("");
  const [dataJogo2, setDataJogo2] = useState<string>("");
  ...
  useEffect(() => {
    ...
    const dataStr = dataDefault.toISOString().slice(0, 16);
    setDataJogo1(dataStr);
    setDataJogo2(dataStr);
  }, []);
  ...
  const timesDisponiveis = vencedoresQuartas.filter(
    (timeId) =>
      ...
      <Card className="w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <CardHeader>
          ...
          <p className="text-center text-gray-600">
            Escolha os confrontos das semifinais entre os vencedores das quartas
            de final
          </p>
        </CardHeader>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    value={semifinal1TimeA}
                    onValueChange={setSemifinal1TimeA}>
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    value={semifinal1TimeB}
                    onValueChange={setSemifinal1TimeB}>
                    <SelectTrigger>
              ...
              {semifinal1TimeA && semifinal1TimeB && (
                <div className="bg-green-50 p-3 rounded-lg text-center">
                  <span className="font-semibold text-green-800">
                    {getTimeNome(semifinal1TimeA)} ×{" "}
                    {getTimeNome(semifinal1TimeB)}
                  </span>
                </div>
              )}
              {/* Campo de data dentro do card da Semifinal 1 */}
              <div>
                <Label htmlFor="data-jogo-1">Data e hora - Semifinal 1</Label>
                <Input
                  id="data-jogo-1"
                  type="datetime-local"
                  value={dataJogo1}
                  onChange={(e) => setDataJogo1(e.target.value)}
                  className="mt-1"
                />
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    value={semifinal2TimeA}
                    onValueChange={setSemifinal2TimeA}>
                    <SelectTrigger>
                      <SelectValue placeholder="Selecione o Time A" />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Select
                    value={semifinal2TimeB}
                    onValueChange={setSemifinal2TimeB}>
                    <SelectTrigger>
                      <SelectValue placeholder="Selecione o Time B" />
                    </SelectTrigger>
                    <SelectContent>
              ...
              {semifinal2TimeA && semifinal2TimeB && (
                <div className="bg-green-50 p-3 rounded-lg text-center">
                  <span className="font-semibold text-green-800">
                    {getTimeNome(semifinal2TimeA)} ×{" "}
                    {getTimeNome(semifinal2TimeB)}
                  </span>
                </div>
              )}
              {/* Campo de data dentro do card da Semifinal 2 */}
              <div>
                <Label htmlFor="data-jogo-2">Data e hora - Semifinal 2</Label>
                <Input
                  id="data-jogo-2"
                  type="datetime-local"
                  value={dataJogo2}
                  onChange={(e) => setDataJogo2(e.target.value)}
                  className="mt-1"
                />
              </div>
            </CardContent>
