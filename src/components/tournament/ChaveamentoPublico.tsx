...
function TeamCard({ timeId, timeName, grupo, logoUrl, isWinner, isLoser, placar, compact = false }: TeamCardProps) {
  if (!timeName) {
    return (
      <div className={`flex items-center gap-2 p-2 bg-muted/50 border border-border rounded-lg ${compact ? 'min-h-[45px]' : 'min-h-[50px]'}`}>
        <div className={`${compact ? 'w-6 h-6' : 'w-8 h-8'} bg-muted rounded-full flex items-center justify-center`}>
  ...
  return (
    <div className={`flex items-center gap-2 p-2 border rounded-lg transition-all ${compact ? 'min-h-[45px]' : 'min-h-[50px]'} ${
      isWinner 
      ...
      <div className={`${compact ? 'w-6 h-6' : 'w-8 h-8'} relative`}>
        {logoUrl ? (
          ...
          <div className={`${compact ? 'w-6 h-6' : 'w-8 h-8'} bg-gradient-to-br from-primary to-primary/80 rounded-full flex items-center justify-center`}>
            <span className={`${compact ? 'text-xs' : 'text-sm'} font-bold text-primary-foreground`}>{timeName?.charAt(0) || '?'}</span>
          </div>
        )}
      </div>
      <div className="flex-1 min-w-0">
        <div className={`font-medium ${compact ? 'text-xs' : 'text-sm'} truncate`}>
          {timeName}
        </div>
        {grupo && !compact && (
...
function Match({ timeA, timeB, grupoA, grupoB, logoA, logoB, placardA, placardB, finalizado, vencedor, compact = false, placeholder = false }: MatchProps) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-green-900 to-slate-900 p-4 rounded-3xl">
      <div className="max-w-7xl mx-auto bg-slate-800/30 backdrop-blur-sm rounded-3xl border border-slate-700/50 shadow-2xl p-6">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-white mb-4">Chaveamento do Campeonato</h1>
          <p className="text-green-200 text-lg">Acompanhe o caminho até a final</p>
          ...
          <div className="mb-8">
            <div className="bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/30 rounded-xl p-6 backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="text-amber-200 font-semibold text-lg">Chaveamento Ilustrativo</h3>
              </div>
              <p className="text-amber-100 leading-relaxed">
                ⚠️ <strong>Atenção:</strong> Este chaveamento pode sofrer alterações, pois a fase de grupos ainda não foi concluída. 
                As posições e confrontos mostrados são baseados na classificação atual e serão definidos oficialmente 
                quando todos os jogos da fase de grupos forem finalizados pelo administrador.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="mb-12">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-white mb-2">Fase de Grupos</h2>
            <p className="text-gray-300">Os 2 primeiros colocados de cada grupo se classificam para as quartas de final</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {['A', 'B', 'C', 'D'].map((grupo) => {
              ...
              const corGrupo = grupo === 'A' ? 'from-yellow-500 to-yellow-600' :
                              grupo === 'B' ? 'from-blue-500 to-blue-600' :
                              grupo === 'C' ? 'from-green-500 to-green-600' :
                              'from-purple-500 to-purple-600';

              return (
                <div key={grupo} className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50 shadow-2xl">
                  <div className={`bg-gradient-to-r ${corGrupo} text-white text-center py-3 rounded-xl mb-4 shadow-lg`}>
                    <h3 className="text-xl font-bold">Grupo {grupo}</h3>
                  </div>
                  
                  <div className="space-y-3">
                    {timesDoGrupo.map((time, timeIndex) => {
                      const timeCompleto = times.find(t => t.id === time.timeId);
                      const isClassificado = timeIndex < 2;
                      
                      return (
                        <div key={`grupo-${grupo}-${time.timeId}-${timeIndex}`} 
                          ...
                          <div className="w-8 h-8 relative">
                            {timeCompleto?.logoUrl ? (
                              ...
                              <div className="w-8 h-8 bg-primary rounded-full flex items-center justify-center">
                                <span className="text-sm font-bold text-primary-foreground">{time.nomeTime?.charAt(0) || '?'}</span>
                              </div>
                            )}
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <span className="font-medium text-white">{time.nomeTime}</span>
                              <span className="text-sm text-gray-300">{time.pontos} pts</span>
                            </div>
                            <div className="text-xs text-gray-400">
                              {time.vitorias}V {time.empates}E {time.derrotas}D | SG: {time.saldoGols > 0 ? '+' : ''}{time.saldoGols}
                            </div>
                          </div>
                          {isClassificado && (
        ...
        <div className="mb-8">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-white mb-2">Fase Eliminatória</h2>
            <p className="text-gray-300">
              {temQuartas ? "Acompanhe os confrontos das eliminatórias" : "Chaveamento será definido após a conclusão da fase de grupos"}
            </p>
          </div>
        </div>
          ...
          <div className="min-w-[1000px] relative">
            ...
            <div className="flex justify-center items-center min-h-[600px] relative">
              ...
              <div className="flex flex-col justify-center space-y-16">
                ...
                {(() => {
                  const jogo = jogosQuartas.find(j => 
                    (classificadosPorGrupo.A[0] && j.timeA === classificadosPorGrupo.A[0].timeId && 
                     classificadosPorGrupo.B[1] && j.timeB === classificadosPorGrupo.B[1].timeId) ||
                    (classificadosPorGrupo.A[0] && j.timeB === classificadosPorGrupo.A[0].timeId && 
                     classificadosPorGrupo.B[1] && j.timeA === classificadosPorGrupo.B[1].timeId)
                  );
                  ...
                  if (jogo) {
                    ...
                    const vencedor = jogo.finalizado ? (
                      (jogo.golsTimeA || 0) > (jogo.golsTimeB || 0) ? jogo.timeA : jogo.timeB
                    ) : undefined;
                    ...
                    return (
                      <div key="quarta-1-placeholder" className="relative">
                        <Match
                          ...
                          grupoA="A"
                          grupoB="B"
                          logoA={time1 ? times.find(t => t.id === time1.timeId)?.logoUrl : undefined}
                          logoB={time2 ? times.find(t => t.id === time2.timeId)?.logoUrl : undefined}
                          placardA={undefined}
                          placardB={undefined}
                          finalizado={false}
                          vencedor={undefined}
                ...
                {(() => {
                  const jogo = jogosQuartas.find(j => 
                    (classificadosPorGrupo.C[0] && j.timeA === classificadosPorGrupo.C[0].timeId && 
                     classificadosPorGrupo.D[1] && j.timeB === classificadosPorGrupo.D[1].timeId) ||
                    (classificadosPorGrupo.C[0] && j.timeB === classificadosPorGrupo.C[0].timeId && 
                     classificadosPorGrupo.D[1] && j.timeA === classificadosPorGrupo.D[1].timeId)
                  );
              ...
              <div className="relative w-16 h-32 mx-4">
                <svg className="w-full h-full" viewBox="0 0 64 128" preserveAspectRatio="none">
                  <path 
                    d="M 0 64 L 64 64" 
                      ...
                      <div className="w-8 h-8 relative">
                        {timeInfo.logo ? (
                          ...
                          <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
                            <span className="text-sm font-bold text-accent">{timeInfo.nome?.charAt(0)}</span>
                          </div>
                        )}
                      </div>
                      <div className="text-accent-foreground font-bold text-lg">CAMPEÃO</div>
                    </div>
                    <div className="text-center text-accent-foreground/90 font-medium mt-1">{timeInfo.nome}</div>
                  </div>
                ) : (
                  <div className="bg-muted px-6 py-3 rounded-full">
                    <div className="text-foreground font-bold text-lg">CAMPEÃO</div>
                  </div>
                )}
                  ...
                  {(() => {
                    if (temFinal && jogoFinal) {
                      ...
                      const vencedor = jogoFinal.finalizado ? (
                        (jogoFinal.golsTimeA || 0) > (jogoFinal.golsTimeB || 0) ? jogoFinal.timeA : jogoFinal.timeB
                      ) : undefined;
              ...
              <div className="flex flex-col justify-center space-y-16">
                ...
                {(() => {
                  const jogo = jogosQuartas.find(j => 
                    (classificadosPorGrupo.B[0] && j.timeA === classificadosPorGrupo.B[0].timeId && 
                     classificadosPorGrupo.A[1] && j.timeB === classificadosPorGrupo.A[1].timeId) ||
                    (classificadosPorGrupo.B[0] && j.timeB === classificadosPorGrupo.B[0].timeId && 
                     classificadosPorGrupo.A[1] && j.timeA === classificadosPorGrupo.A[1].timeId)
                  );
                ...
                {(() => {
                  const jogo = jogosQuartas.find(j => 
                    (classificadosPorGrupo.D[0] && j.timeA === classificadosPorGrupo.D[0].timeId && 
                     classificadosPorGrupo.C[1] && j.timeB === classificadosPorGrupo.C[1].timeId) ||
                    (classificadosPorGrupo.D[0] && j.timeB === classificadosPorGrupo.D[0].timeId && 
                     classificadosPorGrupo.C[1] && j.timeA === classificadosPorGrupo.C[1].timeId)
                  );
        ...
        <div className="mt-16 pt-8 border-t border-white/20">
          <h3 className="text-2xl font-bold text-white text-center mb-6">Classificados por Grupo</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {["A", "B", "C", "D"].map((grupo, index) => {
              ...
              return (
                <div key={grupo} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-lg p-4">
                  <div className={`bg-gradient-to-r ${colors[index]} rounded-lg p-3 mb-3 text-center`}>
                    <h4 className="text-lg font-bold text-white">Grupo {grupo}</h4>
                  </div>
                  <div className="space-y-2">
                    {classificados.map((time, timeIndex) => {
                      const timeCompleto = times.find(t => t.id === time.timeId);
                      return (
                        <div key={`grupo-${grupo}-${time.timeId}-${timeIndex}`} className="flex items-center gap-2 text-sm">
                          <div className="w-5 h-5 relative">
                            {timeCompleto?.logoUrl ? (
                              ...
                              <div className="w-5 h-5 bg-primary rounded-full flex items-center justify-center">
                                <span className="text-xs font-bold text-primary-foreground">{time.nomeTime?.charAt(0) || '?'}</span>
                              </div>
                            )}
                          </div>
                          <span className="flex-1 truncate text-white">{time.nomeTime}</span>
                        </div>
                      );
                    })}
                  </div>
