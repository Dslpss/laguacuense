...
export function obterVencedoresQuartas(jogos: Jogo[]): string[] {
  const quartasFinal = jogos.filter(jogo => jogo.fase === "quartas" && jogo.finalizado);
  ...
  return quartasFinal.map(jogo => {
    ...
    if (jogo.golsTimeA > jogo.golsTimeB) {
      ...
      throw new Error(`Jogo ${jogo.id} terminou empatado - não é possível determinar vencedor`);
    }
  });
}
...
export function obterVencedoresSemifinais(jogos: Jogo[]): string[] {
  const semifinais = jogos.filter(jogo => jogo.fase === "semifinal" && jogo.finalizado);
...
export function obterCampeao(jogos: Jogo[]): string | null {
  ...
  if (final.golsTimeA > final.golsTimeB) {
    ...
    throw new Error(`Final terminou empatada - não é possível determinar campeão`);
  }
}
 ...
 */
export function verificarFaseExiste(jogos: Jogo[], fase: "quartas" | "semifinal" | "final"): boolean {
